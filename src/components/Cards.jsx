...
export const ProfileInfoCard = () =>{
    ...
    const {user, clearUser} = useContext(UserContext);
     ...
     return (
        user && (
             <div className="flex items-center gap-4 px-4 py-2 rounded-2xl bg-white/5 border border-white/20 backdrop-blur-md shadow-md hover:shadow-xl transition-transform hover:scale-[1.03] group">
            {/* Avatar */}
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-purple-600 to-pink-500 flex items-center justify-center text-white text-xl font-semibold shadow-inner group-hover:rotate-[4deg] transition-transform">
                {user.name?.charAt(0).toUpperCase()}
            </div>

            {/* Name + Logout */}
            <div className="flex flex-col leading-snug">
                <span className="text-sm sm:text-base font-semibold text-white truncate max-w-[100px]">
                    {user.name}
                </span>
                <button
                    onClick={handleLogout}
                    className="text-xs sm:text-sm text-pink-400 hover:text-pink-300 transition-colors"
                >
                    Logout
                </button>
            </div>
...
export const TemplateCard = ({thumbnailImg, isSelected , onSelect}) =>{
  return(
    <div className={`group h-auto md:h-[300px] lg:h-[320px] flex flex-col bg-white border-2 overflow-hidden cursor-pointer transition-all duration-500 hover:scale-500 hover:shadow-lg rounded-3xl ${isSelected ? 'border-violet-500 shadow-lg shadow-violet-500/20 bg-violet-50' : 'border-gray-200 hover:border-violet-300'}`} onClick={onSelect}>
      {thumbnailImg ? (
        <div className="relative w-full h-full overflow-hidden">
          <img src={thumbnailImg || 'placeholder.svg'} alt="Template Review" className="w-full h-full object-cover group-hover:scale-110 transition-tranform duration-700" />
